...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(raw_csv_path, "r", encoding="utf-8") as infile, open(
            clean_csv_path, "a", encoding="utf-8", newline=""
        ) as outfile:
            ...
            for row in reader:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Successfully cleaned row with substationID {substation_number}"
                        )
                    )
                    ...
                    self.stderr.write(
                        self.style.ERROR(
                            f"Error cleaning row of substationID: {substation_number}"
                        )
                    )
