    ...
    def handle(self, *args, **options):
        ...
        for level, label in ProposedConnectionVoltageLevel.VOLTAGE_CHOICES:
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.WARNING(f"Already exists: {level} kV")
                )

        if created_count == 0:
            self.stdout.write(
                self.style.NOTICE("No new voltage levels were created.")
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f"{created_count} voltage levels created.")
            )
